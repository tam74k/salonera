...
export function ClientApp() {
        ...
        {showReviewModal && (
          ...
          >
            <motion.div
              ...
              className="bg-stone-900 rounded-[24px] w-full max-w-sm overflow-hidden shadow-2xl"
              dir={isAr ? 'rtl' : 'ltr'}
            >
              <div className="flex justify-between items-center p-6 border-b border-stone-800/50">
                <h3 className="text-xl font-bold text-stone-50">{isAr ? 'تقييم الخدمة' : 'Rate Service'}</h3>
                <button onClick={() => setShowReviewModal(null)} className="p-2 text-stone-500 hover:bg-stone-800 rounded-full transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="p-6 space-y-6 text-center">
                ...
                <div className="text-left" dir={isAr ? 'rtl' : 'ltr'}>
                  <label className="block text-sm font-bold text-stone-300 mb-2">{isAr ? 'ملاحظاتك (اختياري)' : 'Notes (Optional)'}</label>
                  <textarea 
                    value={reviewComment}
                    onChange={(e) => setReviewComment(e.target.value)}
                    className="w-full bg-stone-900/40 border border-stone-800 rounded-xl p-3 text-sm focus:ring-2 focus:ring-amber-500 outline-none resize-none h-24"
                    placeholder={isAr ? 'كيف كانت تجربتك؟' : 'How was your experience?'}
                  />
                </div>
              </div>
              <div className="p-6 border-t border-stone-800/50 bg-stone-900/40 flex gap-3">
                <button onClick={() => setShowReviewModal(null)} className="flex-1 py-3 text-sm font-bold text-stone-400 bg-stone-900 border border-stone-800 rounded-xl hover:bg-stone-800 transition-colors">
                  {isAr ? 'إلغاء' : 'Cancel'}
                </button>
                <button onClick={submitReview} className="flex-1 py-3 text-sm font-bold text-white bg-stone-950 rounded-xl hover:bg-stone-800 transition-colors shadow-md shadow-zinc-900/10">
        ...
        {previewBooking && (
          ...
          >
            <motion.div
              ...
              className="bg-stone-900 rounded-3xl w-full max-w-md overflow-hidden shadow-xl"
            >
              <div className="p-6 border-b border-stone-800/50 flex justify-between items-center">
                <h3 className="font-bold text-xl text-stone-50">{isAr ? 'تفاصيل الحجز' : 'Booking Details'}</h3>
                <button onClick={() => setPreviewBooking(null)} className="p-2 bg-stone-900/40 rounded-full text-stone-500 hover:text-zinc-700 transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="p-6 space-y-6">
                <div>
                  <h4 className="text-stone-400 text-sm font-medium mb-1">{isAr ? 'الصالون' : 'Salon'}</h4>
                  <p className="font-bold text-stone-50 text-lg">{isAr ? previewBooking.salon?.name_ar : previewBooking.salon?.name_en}</p>
                </div>
                <div className="flex justify-between">
                  <div>
                    <h4 className="text-stone-400 text-sm font-medium mb-1">{isAr ? 'التاريخ' : 'Date'}</h4>
                    <p className="font-bold text-stone-50">{previewBooking.booking_date}</p>
                  </div>
                  <div className="text-right">
                    <h4 className="text-stone-400 text-sm font-medium mb-1">{isAr ? 'الوقت' : 'Time'}</h4>
                    <p className="font-bold text-stone-50">{previewBooking.booking_time?.substring(0,5)}</p>
                  </div>
                </div>
                <div>
                  <h4 className="text-stone-400 text-sm font-medium mb-2">{isAr ? 'الخدمات' : 'Services'}</h4>
                  <div className="space-y-2">
                    {previewBooking.details?.map((d: any, idx: number) => (
                      <div key={idx} className="flex justify-between items-center p-3 bg-stone-900/40 rounded-xl">
                        <span className="font-medium text-stone-50">{isAr ? d.services?.name_ar : d.services?.name_en}</span>
                        <span className="text-sm font-bold text-stone-50">{currSymbol} {d.price}</span>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="flex justify-between items-center pt-4 border-t border-stone-800/50">
                  <span className="text-stone-400 font-medium">{isAr ? 'الإجمالي' : 'Total'}</span>
                  <span className="text-2xl font-bold text-stone-50">{currSymbol} {previewBooking.total_amount}</span>
                </div>
    ...
    return (
      <div className="min-h-screen bg-stone-950 font-sans selection:bg-amber-500/30 text-stone-200 pb-24">
        ...
        <motion.div 
          ...
          className="bg-stone-900 rounded-[24px] p-8 border border-stone-800/50 shadow-md shadow-black/20 text-center"
        >
          <div className="w-20 h-20 bg-stone-800/50 text-stone-50 rounded-full flex items-center justify-center mx-auto mb-6">
            <CheckCircle2 className="w-10 h-10" />
          </div>
          <h2 className="text-3xl font-bold text-stone-50 mb-2">{isAr ? 'تم تأكيد الحجز!' : 'Booking Confirmed!'}</h2>
          <p className="text-stone-400 mb-8">
            {isAr ? 'تم حفظ تفاصيل حجزك بنجاح.' : 'Your booking details have been saved successfully.'}
          </p>

          <div className="bg-stone-900/40 rounded-[16px] p-6 mb-8 inline-block text-left w-full max-w-sm" dir={isAr ? 'rtl' : 'ltr'}>
            <div className="flex justify-between items-center mb-4 pb-4 border-b border-stone-800">
              <span className="text-stone-400 font-medium">{isAr ? 'رقم الحجز' : 'Booking ID'}</span>
              <span className="font-bold text-stone-50 uppercase">{bookingConfirmed.split('-')[0]}</span>
            </div>
            <div className="flex justify-between items-center mb-4 pb-4 border-b border-stone-800">
              <span className="text-stone-400 font-medium">{isAr ? 'الوقت والتاريخ' : 'Date & Time'}</span>
              <span className="font-bold text-stone-50">{selectedDate} / {selectedTime}</span>
            </div>
            <div className="flex flex-col mb-4 pb-4 border-b border-stone-800 gap-2">
              <div className="flex justify-between items-center">
                <span className="text-stone-400 font-medium">{isAr ? 'الخدمات المحجوزة' : 'Booked Services'}</span>
                <span className="text-xs font-bold bg-stone-800/50 px-2 py-1 rounded-md">{selectedServices.length}</span>
              </div>
                ...
                {selectedServices.map(srvId => {
                  ...
                  return srv ? (
                    <div key={srvId} className="flex justify-between text-sm">
                      <span className="font-semibold text-stone-200">{isAr ? srv.name_ar : srv.name_en}</span>
                    </div>
                  ) : null;
                })}
              </div>
            ...
            <div className="flex justify-between items-center">
              <span className="text-stone-400 font-medium">{isAr ? 'الإجمالي' : 'Total Amount'}</span>
              <span className="font-bold text-stone-50 text-lg">{currSymbol} {totalPrice}</span>
            </div>
          </div>

          <div className="flex justify-center mb-8">
            <div className="p-4 bg-stone-900 rounded-xl shadow-md shadow-black/20 border border-stone-800/50">
              <QRCodeSVG value={bookingConfirmed} size={150} level="M" />
              <p className="text-xs text-stone-500 mt-3">{isAr ? 'امسح الكود عند الوصول' : 'Scan code upon arrival'}</p>
            </div>
          </div>

          <button onClick={resetFlow} className="w-full max-w-sm bg-stone-950 text-white py-3.5 rounded-xl font-medium hover:bg-stone-800 transition-colors">
  ...
  if (step === 'datetime' && selectedSalon) {
    return (
      <div className="min-h-screen bg-stone-950 font-sans selection:bg-amber-500/30 text-stone-200 pb-24">
        ...
        <button onClick={() => setStep('services')} className="flex items-center gap-2 text-stone-400 hover:text-zinc-900 transition-colors font-medium">
          {isAr ? <ArrowRight className="w-4 h-4" /> : <ArrowLeft className="w-4 h-4" />}
          {isAr ? 'العودة للخدمات' : 'Back to Services'}
        </button>

        <div className="bg-stone-900 rounded-[24px] p-6 md:p-8 border border-stone-800/50 shadow-md shadow-black/20">
          <h2 className="text-2xl font-bold text-stone-50 mb-6">{isAr ? 'اختر الموعد والفني' : 'Select Date & Artist'}</h2>

          {/* Artist Selection */}
          <div className="mb-8">
            <label className="block text-sm font-bold text-stone-300 mb-3 flex items-center gap-2">
              <UserIcon className="w-5 h-5 text-stone-50" />
              {isAr ? 'اختر فني التجميل (اختياري)' : 'Select Artist (Optional)'}
            </label>
            <div className="flex overflow-x-auto gap-3 pb-2 snap-x">
              <div 
                onClick={() => { setSelectedStaff(''); setSelectedTime(''); }}
                className={`flex-shrink-0 snap-start px-6 py-3 rounded-xl border-2 font-bold cursor-pointer transition-colors ${selectedStaff === '' ? 'border-zinc-900 bg-stone-800/50 text-stone-50' : 'border-stone-800/50 bg-stone-900 text-stone-400 hover:border-stone-800'}`}
              >
              ...
              {staff.map(member => (
                <div 
                  ...
                  className={`flex-shrink-0 snap-start px-6 py-3 rounded-xl border-2 font-bold cursor-pointer transition-colors flex items-center gap-3 ${selectedStaff === member.id ? 'border-zinc-900 bg-stone-800/50 text-stone-50' : 'border-stone-800/50 bg-stone-900 text-stone-400 hover:border-stone-800'}`}
                >
          ...
          <div className="mb-8">
            ...
            <input 
              ...
              className="w-full md:w-auto bg-stone-900/40 border border-stone-800 rounded-xl px-4 py-3 focus:ring-2 focus:ring-zinc-900 outline-none font-medium"
            />
          </div>
          ...
          {selectedDate && (
            <div>
              ...
              {availableSlots.length > 0 ? (
                  ...
                  {availableSlots.map(slot => (
                    <button
                      ...
                      className={`py-3 rounded-xl font-bold transition-colors ${
                        !slot.available 
                          ? 'opacity-50 cursor-not-allowed bg-stone-800/50 text-stone-500'
                          : selectedTime === slot.time 
                            ? 'bg-stone-950 text-white shadow-md' 
                            : 'bg-stone-900/40 text-stone-300 hover:bg-stone-800'
                      }`}
                    >
                      {slot.time}
                    </button>
                ...
                <div className="p-4 bg-stone-900/40 text-stone-400 rounded-xl text-center text-sm font-medium">
          ...
          <div className="mt-8 pt-8 border-t border-stone-800/50">
            <button 
              ...
              className="w-full px-8 py-4 bg-stone-950 hover:bg-zinc-900 disabled:bg-slate-300 disabled:cursor-not-allowed text-white rounded-xl font-semibold transition-colors flex items-center justify-center gap-2 text-lg shadow-md"
            >
              {t.book_now}
            </button>
          </div>
  ...
  if (step === 'salon-details' && selectedSalon) {
    return (
      <div className="min-h-screen bg-stone-950 font-sans selection:bg-amber-500/30 text-stone-200 pb-24">
        ...
        {/* Header Carousel */}
        <div className="relative h-64 md:h-96 bg-stone-950 rounded-[24px] overflow-hidden shadow-md shadow-black/20 group">
          {selectedSalon.images && selectedSalon.images.length > 0 ? (
            <>
              <img 
                ...
                className="absolute inset-0 w-full h-full object-contain transition-opacity duration-300 p-2"
              />
              {selectedSalon.images.length > 1 && (
                <>
                  <button onClick={(e) => { e.stopPropagation(); setCurrentImageIndex(prev => prev === 0 ? selectedSalon.images.length - 1 : prev - 1); }} className="absolute left-4 top-1/2 -translate-y-1/2 p-2 bg-stone-900/20 hover:bg-stone-900/40 backdrop-blur-md rounded-full text-white transition-colors">
                    <ChevronLeft className="w-6 h-6" />
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); setCurrentImageIndex(prev => prev === selectedSalon.images.length - 1 ? 0 : prev + 1); }} className="absolute right-4 top-1/2 -translate-y-1/2 p-2 bg-stone-900/20 hover:bg-stone-900/40 backdrop-blur-md rounded-full text-white transition-colors">
                    <ChevronRight className="w-6 h-6" />
                  </button>
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                    {selectedSalon.images.map((_: any, idx: number) => (
                      <div key={idx} className={`w-2 h-2 rounded-full transition-colors ${idx === currentImageIndex ? 'bg-stone-900' : 'bg-stone-900/40'}`} />
                    ))}
                  </div>
              </>
              )}
          ...
          <div className="absolute bottom-6 left-6 right-6">
            ...
            <div className="flex flex-wrap items-center gap-3 text-white/90 text-sm font-medium">
              ...
              {selectedSalon.computedDistance && selectedSalon.computedDistance !== Infinity && (
                <span className="bg-stone-900/20 px-2 py-1 rounded-md">{selectedSalon.computedDistance.toFixed(1)} km</span>
              )}
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <div className="md:col-span-2 space-y-8">
            <section>
              <h3 className="text-xl font-bold text-stone-50 mb-4">{isAr ? 'عن الصالون' : 'About'}</h3>
            ...
            <section>
              <h3 className="text-xl font-bold text-stone-50 mb-4">{isAr ? 'الخدمات المتوفرة' : 'Available Services'}</h3>
              <div className="grid sm:grid-cols-2 gap-4">
                {services.map(srv => (
                  <div key={srv.id} className="p-4 bg-stone-900/40 rounded-xl border border-stone-800/50 flex justify-between items-center">
                    <div>
                      <h4 className="font-bold text-stone-50">{isAr ? srv.name_ar : srv.name_en}</h4>
                      <p className="text-xs text-stone-400 mt-1">{srv.duration_minutes} {isAr ? 'دقيقة' : 'min'}</p>
                    </div>
                    <div className="text-right">
                      {srv.discount_price ? (
                        <>
                          <div className="text-xs text-stone-500 line-through">{currSymbol} {srv.original_price}</div>
                          <div className="font-bold text-stone-50">{currSymbol} {srv.discount_price}</div>
                        </>
                      ) : (
                        <div className="font-bold text-stone-50">{currSymbol} {srv.original_price}</div>
                      )}
                    </div>
                  </div>
                ))}
                {services.length === 0 && <p className="text-stone-400 text-sm">{isAr ? 'لا توجد خدمات مضافة.' : 'No services added.'}</p>}
              </div>
            </section>
            
            <section>
              <h3 className="text-xl font-bold text-stone-50 mb-4">{isAr ? 'فريق العمل' : 'Our Team'}</h3>
              <div className="flex gap-4 overflow-x-auto pb-4 snap-x">
                {staff.map(member => (
                  <div key={member.id} className="snap-start shrink-0 w-32 text-center">
                    <div className="w-20 h-20 mx-auto rounded-full overflow-hidden bg-stone-800/50 mb-3 border-2 border-white shadow-md shadow-black/20">
                      {member.profile?.avatar_url ? (
                         <img src={member.profile.avatar_url} className="w-full h-full object-cover" alt="" />
                      ) : (
                         <div className="w-full h-full flex items-center justify-center text-stone-500"><UserIcon className="w-8 h-8" /></div>
                      )}
                    </div>
                    <h4 className="font-bold text-stone-50 text-sm">{isAr ? member.profile?.first_name_ar : member.profile?.first_name_en}</h4>
                    <p className="text-xs text-stone-400 mt-0.5">{isAr ? 'فني تجميل' : 'Artist'}</p>
                  </div>
                ))}
                {staff.length === 0 && <p className="text-stone-400 text-sm">{isAr ? 'لا يوجد فنيين حالياً.' : 'No artists currently.'}</p>}
              </div>
            </section>
          </div>

          <div className="md:col-span-1">
            <div className="sticky top-6 bg-stone-900 border border-stone-800/50 p-6 rounded-[24px] shadow-md shadow-black/20">
              <h3 className="font-bold text-stone-50 text-lg mb-4">{isAr ? 'حجز موعد' : 'Book Appointment'}</h3>
              <p className="text-sm text-stone-400 mb-6">{isAr ? 'احجز موعدك الآن واستمتع بخدماتنا.' : 'Book your appointment now and enjoy our services.'}</p>
              <button onClick={() => setStep('services')} className="w-full py-4 text-white bg-stone-950 hover:bg-stone-800 font-bold rounded-xl transition-colors shadow-md flex items-center justify-center gap-2">
  ...
  if (step === 'services' && selectedSalon) {
    return (
      <div className="min-h-screen bg-stone-950 font-sans selection:bg-amber-500/30 text-stone-200 pb-24">
        <div className="max-w-3xl mx-auto p-4 md:p-8 space-y-6">
        <button onClick={() => setSelectedSalon(null)} className="flex items-center gap-2 text-stone-400 hover:text-zinc-900 transition-colors font-medium">
          {isAr ? <ArrowRight className="w-4 h-4" /> : <ArrowLeft className="w-4 h-4" />}
          {isAr ? 'العودة للصالونات' : 'Back to Salons'}
        </button>

        <div className="bg-stone-900 rounded-[24px] overflow-hidden border border-stone-800/50 shadow-md shadow-black/20">
          <div className="h-48 md:h-64 relative bg-stone-800/50">
          ...
          <div className="p-6 md:p-8">
            <h3 className="text-xl font-bold text-stone-50 mb-6">{isAr ? 'اختر الخدمات' : 'Select Services'}</h3>
              ...
              {services.map(service => {
                ...
                return (
                  <div 
                    ...
                    className={`p-4 rounded-[16px] border-2 transition-all cursor-pointer flex justify-between items-center ${isSelected ? 'border-zinc-900 bg-stone-800/50' : 'border-stone-800/50 hover:border-stone-800 bg-stone-900'}`}
                  >
                    <div>
                      <h4 className={`font-bold ${isSelected ? 'text-stone-50' : 'text-stone-50'}`}>
                        {isAr ? service.name_ar : service.name_en}
                      </h4>
                    </div>
                    <span className={`font-bold ${isSelected ? 'text-stone-50' : 'text-stone-400'} flex flex-col items-end`}>
                      {service.discount_price ? (
                        <>
                          <span className="text-xs line-through opacity-50">{currSymbol} {service.original_price}</span>
                          <span className="text-stone-50">{currSymbol} {service.discount_price}</span>
                        </>
                      ) : (
            ...
            <div className="mt-8 pt-8 border-t border-stone-800/50 flex flex-col md:flex-row items-center justify-between gap-4">
              <div>
                <p className="text-stone-400 font-medium">{isAr ? 'إجمالي السعر' : 'Total Price'}</p>
                <p className="text-3xl font-bold text-stone-50">{currSymbol} {totalPrice}</p>
              </div>
              <button 
                onClick={handleNextToDateTime}
                disabled={selectedServices.length === 0}
                className="w-full md:w-auto px-8 py-3.5 bg-stone-950 hover:bg-zinc-900 disabled:bg-slate-300 disabled:cursor-not-allowed text-white rounded-xl font-semibold transition-colors flex items-center justify-center gap-2"
              >
              ...
              {favoriteSalons.map((salon, idx) => (
                ...
                >
                  ...
                  {(salon.images?.[0] || salon.image_url) ? (
                    <img src={salon.images?.[0] || salon.image_url} alt="" className="absolute inset-0 w-full h-full object-contain transition-all duration-1000 ease-out p-4" />
                  ) : (
  ...
  if (step === 'profile') {
    return (
      <div className="min-h-screen bg-stone-950 font-sans selection:bg-amber-500/30 text-stone-200 pb-24">
        ...
        <button onClick={() => setStep('salons')} className="flex items-center gap-2 text-stone-400 hover:text-zinc-900 transition-colors font-medium">
          {isAr ? <ArrowRight className="w-4 h-4" /> : <ArrowLeft className="w-4 h-4" />}
          {isAr ? 'العودة للصالونات' : 'Back to Salons'}
        </button>
        <div className="bg-stone-900 rounded-[24px] overflow-hidden border border-stone-800/50 shadow-md shadow-black/20 p-6 md:p-8">
          <div className="flex items-center gap-4 mb-8 pb-6 border-b border-stone-800/50">
            <div className="w-16 h-16 bg-stone-800/50 rounded-full flex items-center justify-center text-stone-500">
              <UserIcon className="w-8 h-8" />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-stone-50">{isAr ? 'حسابي' : 'My Profile'}</h2>
          ...
          <div className="space-y-5">
            <div className="grid md:grid-cols-2 gap-5">
              <div>
                <label className="block text-sm font-medium text-stone-300 mb-1.5">{isAr ? 'الاسم (عربي)' : 'First Name (Arabic)'}</label>
                <input type="text" value={profileData.first_name_ar} onChange={e => setProfileData({...profileData, first_name_ar: e.target.value})} className="w-full bg-stone-900/40 border border-stone-800 rounded-xl px-4 py-3 outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-stone-300 mb-1.5">{isAr ? 'الاسم (إنجليزي)' : 'First Name (English)'}</label>
                <input type="text" value={profileData.first_name_en} onChange={e => setProfileData({...profileData, first_name_en: e.target.value})} className="w-full bg-stone-900/40 border border-stone-800 rounded-xl px-4 py-3 outline-none" />
              </div>
            </div>
            
            <div>
              <label className="block text-sm font-medium text-stone-300 mb-1.5">{isAr ? 'رقم الجوال' : 'Mobile Number'}</label>
            ...
            <div className="pt-4 border-t border-stone-800/50">
              <label className="block text-sm font-medium text-stone-300 mb-1.5">{isAr ? 'كلمة المرور الجديدة (اختياري)' : 'New Password (Optional)'}</label>
              <div className="relative">
                <Lock className="w-5 h-5 text-stone-500 absolute top-3.5 left-4" />
                <input 
                  type="password" 
                  value={newPassword}
                  onChange={e => setNewPassword(e.target.value)}
                  placeholder={isAr ? 'اتركه فارغاً إذا لم ترغب بتغييره' : 'Leave empty to keep current'}
                  className="w-full bg-stone-900/40 border border-stone-800 rounded-xl pl-12 pr-4 py-3 outline-none" 
                />
              </div>
            </div>
            ...
            <div className="pt-6">
              <button 
                ...
                className="w-full bg-stone-950 text-white rounded-xl py-3.5 font-bold flex items-center justify-center gap-2 hover:bg-stone-800 disabled:opacity-70 transition-all"
              >
  ...
  if (step === 'my-bookings') {
    ...
    return (
      ...
      <div className="max-w-3xl mx-auto p-4 md:p-8 space-y-6">
        ...
        <h2 className="text-3xl font-bold text-stone-50 mb-6">{isAr ? 'حجوزاتي' : 'My Bookings'}</h2>

        <div className="flex bg-stone-800/50 p-1.5 rounded-xl mb-6">
          <button onClick={() => setBookingFilter('current')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-colors ${bookingFilter === 'current' ? 'bg-stone-900 text-stone-50 shadow-md shadow-black/20' : 'text-stone-400 hover:text-zinc-900'}`}>
            {isAr ? 'الحالية' : 'Current'}
          </button>
          <button onClick={() => setBookingFilter('past')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-colors ${bookingFilter === 'past' ? 'bg-stone-900 text-stone-50 shadow-md shadow-black/20' : 'text-stone-400 hover:text-zinc-900'}`}>
            {isAr ? 'السابقة' : 'Past'}
          </button>
          <button onClick={() => setBookingFilter('canceled')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-colors ${bookingFilter === 'canceled' ? 'bg-stone-900 text-stone-50 shadow-md shadow-black/20' : 'text-stone-400 hover:text-zinc-900'}`}>
        ...
        <div className="space-y-4">
          {filteredBookings.length === 0 ? (
            ...
            filteredBookings.map(b => {
              ...
              return (
                <div key={b.id} className="bg-stone-900 p-6 rounded-[24px] border border-stone-800/50 shadow-md shadow-black/20 flex flex-col md:flex-row gap-4 justify-between">
                  <div>
                    <h3 className="font-bold text-lg text-stone-50 mb-1">{isAr ? b.salon?.name_ar : b.salon?.name_en}</h3>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {b.details?.map((d: any, idx: number) => (
                        <span key={idx} className="bg-stone-800/50 text-stone-300 text-xs px-2 py-1 rounded-md font-medium">
                  ...
                  <div className="flex flex-col justify-between items-end gap-3">
                    ...
                    {b.status === 'confirmed' && (
                       <span className="text-xs text-stone-500 font-mono">ID: {b.id.substring(0,8)}</span>
                    )}
                    <div className="flex items-center gap-2 mt-2 md:mt-0">
                      <button 
                        onClick={() => setPreviewBooking(b)}
                        className="text-xs font-bold px-3 py-1.5 rounded-lg bg-stone-900/40 text-stone-400 hover:bg-stone-800 transition-colors border border-stone-800 flex items-center gap-1.5"
                      >
  ...
  return (
      <div className="min-h-screen bg-stone-950 font-sans selection:bg-amber-500/30 text-stone-200"><div className="max-w-5xl mx-auto p-4 md:p-8 space-y-8 pb-24">
      {/* Hero Section */}
      <section className="relative bg-stone-950 rounded-[24px] overflow-hidden p-8 md:p-12 text-white">
        ...
        <div className="relative z-10 max-w-lg">
          ...
          <div className="flex flex-wrap gap-3">
            <button onClick={() => { fetchMyBookings(); setStep('my-bookings'); }} className="mt-4 bg-white/10 hover:bg-white/20 text-white px-5 py-2.5 rounded-full text-sm font-bold transition-colors">
              {isAr ? 'حجوزاتي' : 'Bookings'}
            </button>
            <button onClick={() => setStep('favorites')} className="mt-4 bg-white/10 hover:bg-white/20 text-white px-5 py-2.5 rounded-full text-sm font-bold transition-colors">
              {isAr ? 'المفضلة' : 'Favorites'}
            </button>
            <button onClick={() => { fetchUserProfile(); setStep('profile'); }} className="mt-4 bg-white/10 hover:bg-white/20 text-white px-5 py-2.5 rounded-full text-sm font-bold transition-colors">
              {isAr ? 'حسابي' : 'Profile'}
            </button>
          </div>
          <div className="flex bg-white/10 backdrop-blur-md rounded-full p-1.5 mt-6 border border-white/20">
            <button 
              ...
              className={`flex-1 rounded-full py-2.5 text-sm font-semibold transition-colors ${filterSalonType === 'men' ? 'bg-stone-900 text-stone-50 shadow-md shadow-black/20' : 'text-white hover:bg-white/10'}`}
            >
              {t.men_salons}
            </button>
            <button 
              onClick={() => setFilterSalonType('women')}
              className={`flex-1 rounded-full py-2.5 text-sm font-semibold transition-colors ${filterSalonType === 'women' ? 'bg-stone-900 text-stone-50 shadow-md shadow-black/20' : 'text-white hover:bg-white/10'}`}
            >
              {t.women_salons}
            </button>
          </div>
      ...
      <section>
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 gap-4">
          <div>
            <h3 className="text-xl font-bold text-stone-50">{t.discover_salons}</h3>
            {locationError && (
          ...
          {!userLocation && (
            <button 
              ...
              className="text-sm font-medium text-stone-50 flex items-center gap-1 hover:bg-stone-800 px-3 py-1.5 rounded-lg transition-colors"
            >
              <MapPin className="w-4 h-4" />
        ...
        {favorites.length > 0 && (
          <div className="mb-8">
            <h3 className="text-xl font-bold text-stone-50 mb-4 flex items-center gap-2">
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 snap-x custom-scrollbar">
              {salons.filter(s => favorites.includes(s.id)).map(salon => (
                <motion.div 
                  ...
                  animate={{ opacity: 1, scale: 1 }}
                  onClick={() => { setSelectedSalon(salon); setCurrentImageIndex(0); setStep('salon-details'); fetchSalonDetails(salon.id); }}
                  className="snap-start shrink-0 w-[280px] bg-stone-900 rounded-[20px] overflow-hidden border border-stone-800/50 shadow-md shadow-black/20 cursor-pointer hover:shadow-md transition-all group relative"
                >
                  <div className="h-36 relative bg-stone-800/50">
                    {(salon.images?.[0] || salon.image_url) && <img src={salon.images?.[0] || salon.image_url} alt="" className="w-full h-full object-contain transition-transform duration-500 p-2" />}
                    <button onClick={(e) => toggleFavorite(salon.id, e)} className="absolute top-3 left-3 p-2 bg-stone-950/90 hover:bg-stone-900 backdrop-blur-md rounded-full shadow-md shadow-black/20 text-rose-500 transition-colors z-10">
                      <Heart className="w-4 h-4 fill-rose-500" />
                    </button>
                    <div className="absolute top-3 right-3 bg-stone-900/95 backdrop-blur-sm px-2 py-1 rounded-lg flex items-center gap-1 text-xs font-bold text-stone-200 z-10 shadow-md shadow-black/20">
                      <Star className="w-3.5 h-3.5 text-amber-500 fill-amber-500" />
                      {salon.avgRating ? salon.avgRating.toFixed(1) : (salon.rating || 'New')}
                    </div>
                  </div>
                  <div className="p-4">
                    <h4 className="font-bold text-stone-50 text-[15px] truncate mb-1">{isAr ? salon.name_ar : salon.name_en}</h4>
        ...
        {/* Filters */}
        <div className="bg-stone-900 p-6 rounded-[24px] shadow-md shadow-black/20 border border-stone-800/50 mb-8">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-bold text-stone-50 flex items-center gap-2">
              <MapPin className="w-5 h-5 text-stone-50" />
              {isAr ? 'تصفية الصالونات' : 'Filter Salons'}
            </h3>
            <div className="flex items-center gap-2 bg-stone-900/40 p-1 rounded-xl border border-stone-800">
              <button onClick={() => setViewMode('grid')} className={`p-1.5 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-stone-900 shadow-md shadow-black/20 text-stone-50' : 'text-stone-500 hover:text-stone-400'}`}>
                <GridIcon className="w-4 h-4" />
              </button>
              <button onClick={() => setViewMode('map')} className={`p-1.5 rounded-lg transition-colors ${viewMode === 'map' ? 'bg-stone-900 shadow-md shadow-black/20 text-stone-50' : 'text-stone-500 hover:text-stone-400'}`}>
          ...
          <div className="grid md:grid-cols-4 gap-4">
            <div>
              <select
                ...
                className="w-full bg-stone-900/40 border border-stone-800 rounded-xl px-4 py-2.5 outline-none font-medium text-sm"
              >
            ...
            <div>
              <select value={filterCountry} onChange={e => { const val = e.target.value; setFilterCountry(val); setFilterGov(''); setFilterCity(''); updateProfileLocation(val, '', ''); }} className="w-full bg-stone-900/40 border border-stone-800 rounded-xl px-4 py-2.5 outline-none font-medium">
            ...
            <div>
              <select value={filterGov} onChange={e => { const val = e.target.value; setFilterGov(val); setFilterCity(''); updateProfileLocation(filterCountry, val, ''); }} disabled={!filterCountry} className="w-full bg-stone-900/40 border border-stone-800 rounded-xl px-4 py-2.5 outline-none font-medium disabled:opacity-50">
            ...
            <div>
              <select value={filterCity} onChange={e => { const val = e.target.value; setFilterCity(val); updateProfileLocation(filterCountry, filterGov, val); }} disabled={!filterGov} className="w-full bg-stone-900/40 border border-stone-800 rounded-xl px-4 py-2.5 outline-none font-medium disabled:opacity-50">
        ...
        {(() => {
          ...
          if (viewMode === 'map') {
             ...
             return (
                <div className="relative w-full h-[600px] bg-stone-900/50 border border-stone-800 rounded-[24px] overflow-hidden shadow-md shadow-black/20">
                   ...
                   {userLocation && (
                      <div className="absolute transform -translate-x-1/2 -translate-y-1/2 z-10 flex flex-col items-center" style={{ top: getTop(userLocation.lat), left: getLeft(userLocation.lng) }}>
                        ...
                        <span className="text-[10px] font-bold bg-stone-900 px-2 py-0.5 rounded-full mt-1 shadow-md shadow-black/20 text-stone-300">{isAr ? 'موقعك' : 'You'}</span>
                      </div>
                   )}
                   ...
                   {validMapSalons.map(salon => (
                      <div key={salon.id} onClick={() => { setSelectedSalon(salon); setCurrentImageIndex(0); setStep('salon-details'); fetchSalonDetails(salon.id); }} className="absolute transform -translate-x-1/2 -translate-y-1/2 cursor-pointer group z-20 flex flex-col items-center" style={{ top: getTop(salon.lat), left: getLeft(salon.lng) }}>
                        <MapPin className="w-8 h-8 text-stone-50 fill-white transition-transform group-hover:scale-125 group-hover:text-amber-500 drop-shadow-md" />
                        <div className="absolute top-full mt-1 bg-stone-900 px-3 py-1.5 rounded-xl shadow-lg border border-stone-800/50 text-xs font-bold whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none flex flex-col items-center z-30">
                           {isAr ? salon.name_ar : salon.name_en}
                           <div className="text-[10px] text-stone-400 font-medium mt-0.5 flex justify-center items-center gap-1">
          ...
          return (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              className="bg-stone-900 rounded-[16px] overflow-hidden border border-stone-800/50 shadow-md shadow-black/20 hover:shadow-md transition-all group flex flex-col"
            >
              <div className="h-48 overflow-hidden relative bg-stone-800/50">
                {(salon.images?.[0] || salon.image_url) && <img src={salon.images?.[0] || salon.image_url} alt={isAr ? salon.name_ar : salon.name_en} className="w-full h-full object-contain transition-transform duration-500 p-2" />}
                
                <button onClick={(e) => toggleFavorite(salon.id, e)} className="absolute top-3 left-3 p-2 bg-stone-950/90 hover:bg-stone-900 backdrop-blur-md rounded-full transition-colors z-10 shadow-md shadow-black/20">
                  <Heart className={`w-4 h-4 ${favorites.includes(salon.id) ? 'text-rose-500 fill-rose-500' : 'text-stone-500'}`} />
                </button>
                <div className="absolute top-3 right-3 bg-stone-950/90 backdrop-blur-sm px-2 py-1 rounded-md flex items-center gap-1 text-xs font-bold text-stone-200">
                  <Star className="w-3.5 h-3.5 text-amber-500 fill-amber-500" />
                  {salon.avgRating ? salon.avgRating.toFixed(1) : (salon.rating || 'New')}
                </div>
              </div>
              <div className="p-5 flex flex-col flex-1">
                <div className="flex justify-between items-start mb-2 gap-2">
                  <h4 className="text-lg font-bold text-stone-50">{isAr ? salon.name_ar : salon.name_en}</h4>
                  {salon.computedDistance && salon.computedDistance !== Infinity ? (
                    <span className="text-xs font-bold text-stone-50 bg-stone-800/50 px-2 py-1 rounded-full flex items-center gap-1 shrink-0">
                      <MapPin className="w-3 h-3" />
                      {salon.computedDistance.toFixed(1)} km
                    </span>
                  ) : (
                    <span className="text-xs font-medium text-stone-400 bg-stone-800/50 px-2 py-1 rounded-full shrink-0">{salon.location_text || (salon.city || 'Nearby')}</span>
                  )}
                </div>
                ...
                <div className="mt-auto grid grid-cols-2 gap-3 pt-4 border-t border-zinc-50">
                  <button onClick={() => { setSelectedSalon(salon); setCurrentImageIndex(0); setStep('salon-details'); fetchSalonDetails(salon.id); }} className="w-full py-2.5 text-sm font-bold text-stone-300 bg-stone-900/40 hover:bg-stone-800 rounded-xl transition-colors">
                    {isAr ? 'عرض' : 'View'}
                  </button>
                  <button onClick={() => { setSelectedSalon(salon); setCurrentImageIndex(0); setStep('services'); fetchSalonDetails(salon.id); }} className="w-full py-2.5 text-sm font-bold text-white bg-stone-950 hover:bg-stone-800 rounded-xl transition-colors shadow-md shadow-black/20">
                    {isAr ? 'حجز' : 'Book'}
                  </button>
                </div>
              </div>
