...
const targetRegex = /<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">\s*\{salons\.filter\([\s\S]*?\}\)\s*\.map\(\(salon, i\) => \(\s*<motion\.div/m;
...
const emptyBlockRegex = /\{salons\.length === 0 && !isLoadingData && \([\s\S]*?\}\s*<\/div>\s*<\/section>/m;
const emptyBlockReplacement = `{processedSalons.length === 0 && !isLoadingData && (
...
content = content.replace(emptyBlockRegex, emptyBlockReplacement);
